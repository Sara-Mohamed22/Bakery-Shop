...
class _DetailsOrderState extends State<DetailsOrder>{
  ...
  Future<void> createOrder (Map<String,dynamic> data) async {
    OrderModel order=OrderModel(
        userId: context.read<UserProvider>().userModel.userid,
        idProduct: widget.idProduct,
        productname:data['productname'],
        image:data['image'],
        quantity:quantity.toString(),
        description: Provider.of<AdminProvider>(context,listen: false).noteProductController.text);
    await FirestoreHelper.firestoreHelper.createOrder(order,context);
  }
                          ...
                          height: 50,
                            child: Center(
                              child: ElevatedButton(
                                onPressed:()=> createOrder(snapshot.data.data()),
                                child:const Text(
                                'Make Order',
                                 textScaleFactor: 2,
                                 style: TextStyle(
