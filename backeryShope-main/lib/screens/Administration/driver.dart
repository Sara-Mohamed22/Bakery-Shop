...
class _DriverState extends State<Driver> {
  ...
  Future<void> updateOrder(String orderId) async {
    //await FirestoreHelper.firestoreHelper.changeOrderState(orderStatus.delivered, orderId, DateTime.now().toIso8601String());
    await FirestoreHelper.firestoreHelper.changeOrderState(orderStatus.complete, orderId, DateTime.now().toIso8601String());


    Fluttertoast.showToast(
      msg: "order Deliver now",
    ...
    return InkWell(
        child: Container(
          ...
          decoration: BoxDecoration(
            color: typeSelected == title ? Colors.green : const Color(0xFF3E2723),
  ...
  Widget build(BuildContext context) {
   ...
   /* CollectionReference citiesRef = FirebaseFirestore.instance.collection("orders");
    citiesRef.where("status" , whereIn: [orderStatus.preparing.name, orderStatus.ready.name] ); */
    ...
    return SafeArea(
        child: Scaffold(
            ...
            body: SafeArea(
              ...
              Selector<AdminProvider, int>(builder: (context, value, widget) {
                return Row(
                  children: [
                    Expanded(
                      child: Container(
              ...
              StreamBuilder<QuerySnapshot>(
                  ...
                  stream: FirebaseFirestore.instance
                      .collection('orders')
                      //.where('status', isEqualTo: orderStatus.preparing.name   )
                      // .where('status', isEqualTo: orderStatus.ready.name )
                    // .where('status'  ,whereIn: [orderStatus.preparing.name ,orderStatus.ready.name])
                      .where('status'  ,whereIn: [orderStatus.delivered.name ,orderStatus.ready.name])
                                    ...
                                    return Container(
                                      ...
                                      child: Container(
                                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                          Text('Order Details:', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.red)),
                                          Text(' product name: ' + snapshot.data.docs[index]["productname"] ?? 'none',
                                          ...
                                          Container(
                                              child: StreamBuilder<QuerySnapshot>(
                                                  stream: FirebaseFirestore.instance
                                                      .collection('users')
                                                      // .where('groupName', isEqualTo:provider.gruopName)
                                                      .where('userid', isEqualTo: snapshot.data.docs[index]["userID"])
                                                      .snapshots(),
                                                    ...
                                                    if (!snapshot.hasData) {
                                                      ...
                                                      return (snapshot.data.docs.isEmpty)
                                                          ? Center(
                                                              child: Text('\There are no products to deliver',
                                                                  style:
                                                                      TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF3E2723))))
                                                          : ListView.builder(
                                                              physics: NeverScrollableScrollPhysics(),
                                                              shrinkWrap: true,
                                                              scrollDirection: Axis.vertical,
                                            ...
                                            children: [
                                              Container(
                                                ...
                                                child: ElevatedButton(
                                                  onPressed: /*shouldEnable ? null :*/() async {
                                                    ...
                                                    await FirestoreHelper.firestoreHelper.changeOrderState(orderStatus.delivered, snapshot.data.docs[index].id) ;

                                                    Fluttertoast.showToast(
                                                      msg: "start Deliver now",
                                                      textColor: Colors.white,
                                                      toastLength: Toast.LENGTH_SHORT,
                                                      gravity: ToastGravity.BOTTOM,
                                                      backgroundColor: Colors.indigo,
                                              ...
                                              Container(
                                                ...
                                                width: 100,
                                                child: ElevatedButton(
                                                  onPressed: () => updateOrder(snapshot.data.docs[index].id)
                                                  ,
                                                  child: const Text(
                                                    'Delivered',
                                                    style: TextStyle(fontSize: 12),
