...
class FirestoreHelper {
 ...
 String userGroup;
  ...
  DocumentReference<Map<String, dynamic>> makeID;
 ...
 //  }
  ...
  makeProductOrders(OrderModel orderModel,context) async{
     ...
     OrderModel model =OrderModel(
      groupName: userModel.group,
        productname: orderModel.productname,description: orderModel.description,image: orderModel.image,
        userId:userId, quantity: orderModel.quantity,orderType: orderModel.orderType,idProduct:orderModel.idProduct );

   makeID= await firestore.collection('orders').add(model.toMap());
   await firestore.collection('orders').doc(makeID.id)
        .update({'id':makeID.id,"orderType":"begin",'userID':userId,});
      // 'quantity':orderModel.quantity,'description':orderModel.description});
