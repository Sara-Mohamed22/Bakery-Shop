...
int group;
 ...
 await firestore
  ...
  sendPreparedOrderToDriver(QueryDocumentSnapshot<Object>  driverOrder,context,String userID)async{
    ...
    // );
    await FirestoreHelper.firestoreHelper.addPrepareOrderToDriver(OrderModel.toMap(driverOrder),context,userID);
    notifyListeners();
  }
  ...
  stopDriver(DriverModel orderModel,String orderID)async{
   ...
   makeProductOrderP(OrderModel orderModel,context) async {
    ...
    await FirestoreHelper.firestoreHelper.makeProductOrders(orderModel,context);
    clearVariablesOrder();
    notifyListeners();
  ...
  makeOrder(QueryDocumentSnapshot<Object> orderModel) async {
    ...
    await FirestoreHelper.firestoreHelper.makeOrders(OrderModel.toMap(orderModel));
    notifyListeners();
